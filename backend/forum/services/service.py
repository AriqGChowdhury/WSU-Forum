...
#Saves updated user, returns role for patch, also returns info to see current settings
class SettingsService:
    ...
    def save_user(self, name):
        return self.__save_user(name)

    def update_profile(self):
        return self.__get_role()[0]

    #Private
    def __save_user(self, name):
        parts = name.strip().split(" ", 1)
        self.__user.first_name = parts[0]
        self.__user.last_name = parts[1] if len(parts) > 1 else ""
        self.__user.save()
...
#Allows you to like and unlike a post
class LikeService:
...
#Allows you to delete your comment made on a post
...
#Allows you to view single post 
...
#Allows you to save and unsave post
class SaveService:
    def __init__(self, user, post_id):
        self.__user = user
        self.__postID = post_id
...
#Returns posts, commented on, saved posts, following and followers for a user
