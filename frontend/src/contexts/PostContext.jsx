...
const normalizePosts = (posts) =>
  posts.map((p) => ({
    ...p,
    likes: Number(p.likes ?? p.like_amt ?? 0),
    comment_count: Number(p.comments ?? p.comment_amt ?? 0),
    liked: Boolean(p.liked),
    saved: Boolean(p.saved ?? p.is_saved ?? false),
  }));
...
export function PostProvider({ children }) {
  ...
  const [includeMockData, setIncludeMockData] = useState(false); // Toggle for mock data
    ...
    api.posts.getAll()
      .then(({ posts: realPosts }) => {
        if (!cancelled) {
          ...
          const normalized = normalizePosts(realPosts || []);
          const allPosts = includeMockData 
            ? [...(normalized || []), ...MOCK_POSTS]
            : (normalized || []);
      ...
      setPosts((prev) =>
        prev.map((p) => {
          ...
          return {
            ...p,
            liked: !p.liked,
            likes: !p.liked ? currentLikes + 1: Math.max(0, currentLikes - 1),
          };
        })
          // p.id === id
          //   ? { ...p, liked: !p.liked, likes: p.liked ? p.likes - 1 : p.likes + 1 }
  ...
  const refreshPosts = useCallback(async () => {
    ...
    try {
      ...
      // const normalizedRealPosts = (realPosts || []).map((p) => ({
      //   ...p,
      //   likes: p.likes ?? p.like_amt ?? 0,
      // }));
      const normalized = normalizePosts(realPosts || []);
      const allPosts = includeMockData 
        ? [...normalized, ...MOCK_POSTS]
